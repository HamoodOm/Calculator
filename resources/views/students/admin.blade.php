...
<head>
  ...
  <style>
    ...
    .box { position:absolute; border:2px dashed rgba(20,184,166,.9); border-radius:8px; cursor:move; user-select:none; }
    .box .handle { position:absolute; bottom:-8px; right:-8px; width:12px; height:12px; border-radius:50%; background:#14b8a6; cursor:nwse-resize; }
    .box .tag { position:absolute; top:-22px; left:0; font-size:11px; background:#fff; border:1px dashed rgba(20,184,166,.5); padding:0 6px; border-radius:6px; }
    .editor-shell { width:fit-content; max-width:none; }
    .editor-scroll { overflow:auto; -webkit-overflow-scrolling:touch; background:#f8fafc; border:1px solid #5eead4; border-radius:12px; padding:10px; }
    ...
    #grid { position:absolute; left:0; top:0; right:0; bottom:0; pointer-events:none; display:none;
            background-image: repeating-linear-gradient(0deg, rgba(20,184,166,.12), rgba(20,184,166,.12) 1px, transparent 1px, transparent 20px),
                              repeating-linear-gradient(90deg, rgba(20,184,166,.12), rgba(20,184,166,.12) 1px, transparent 1px, transparent 20px); }
    #preview-iframe { width:100%; height:560px; border:1px solid #5eead4; border-radius:10px; background:#fff; }
...
<body class="min-h-screen text-gray-900">
  ...
  <div class="max-w-6xl mx-auto p-4 space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-xl font-bold">إصدار شهادات الطلاب</h1>
      @if(auth()->user()->canAccessStudentSimple())
      <a href="{{ route('students.index') }}" class="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 text-sm">
        الواجهة المبسطة
      </a>
      @endif
    </div>
    ...
    @if(session('success'))
      <div class="bg-green-50 border border-green-200 text-green-800 p-3 rounded mb-3 flex items-center justify-between">
        ...
        @if(session('download_url'))
          <a href="{{ session('download_url') }}" class="btn bg-green-600 text-white hover:bg-green-700">تحميل الملف</a>
        @endif
      </div>
    @endif
    ...
    {{-- ===== ONE FORM (Preview + Generate) ===== --}}
    <form id="students-form"
          action="{{ route('students.admin.store') }}"
          method="post"
      ...
      <div class="grid md:grid-cols-4 gap-4">
        <div>
          ...
          <div class="flex gap-2">
            ...
            <!-- <button type="button" onclick="openAddTrackModal()" class="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 whitespace-nowrap" title="إضافة مسار جديد">+</button>
            <button type="button" onclick="deleteSelectedTrack()" class="px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700 whitespace-nowrap" title="حذف المسار المحدد">×</button> -->
          </div>
        </div>
        ...
        <div>
            ...
            <div class="flex gap-1 flex-wrap">
              <button type="button" onclick="openAddTrackModal()" class="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 whitespace-nowrap text-sm" title="إضافة مسار جديد">إضافة</button>
              @if(auth()->user()->hasPermission(\App\Models\Permission::TRACKS_EDIT))
              <button type="button" onclick="openEditTrackModal()" class="px-3 py-2 bg-amber-600 text-white rounded hover:bg-amber-700 whitespace-nowrap text-sm" title="تعديل المسار">تعديل</button>
              @endif
              <button type="button" onclick="deleteSelectedTrack()" class="px-3 py-2 bg-red-600 text-white rounded hover:bg-red-700 whitespace-nowrap text-sm" title="حذف المسار المحدد">حذف</button>
            </div>
        </div>
        ...
        <div class="md:col-span-2">
          <label class="block text-sm font-medium mb-1">ملف الطلاب (CSV أو Excel)</label>
          <input type="file" name="students_file" accept=".csv,.xlsx" class="w-full border rounded px-3 py-2" required>
          <p class="text-xs text-gray-500 mt-1">الأعمدة: name_ar, name_en (اختياريًا student_id, photo_filename)</p>
          <p class="text-xs text-blue-700 mt-1">
            <a class="underline" href="{{ route('students.template','csv') }}">قالب CSV</a> ·
            <a class="underline" href="{{ route('students.template','xlsx') }}">قالب Excel</a>
          </p>
        </div>

        <div>
          <label class="block text-sm font-medium mb-1">صور المتدرّبين (ZIP) — اختياري</label>
          <input type="file" name="images_zip" id="images_zip" accept=".zip" class="w-full border rounded px-3 py-2">
          <p class="text-xs text-gray-500 mt-1">أسماء مثل: {student_id}.jpg|jpeg|png|webp — أو photo_filename.</p>
          <div class="mt-2">
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <button type="submit" class="btn bg-green-600 text-white hover:bg-green-700">إنشاء شهادات PDF (ZIP)</button>

        {{-- Image generation buttons --}}
        <button type="button" id="btn-preview-image" class="btn bg-purple-600 text-white hover:bg-purple-700">معاينة صورة</button>
        <button type="button" id="btn-generate-images" class="btn bg-teal-600 text-white hover:bg-teal-700">إنشاء شهادات صور (ZIP)</button>
        ...
        <button type="button" id="btn-clear-server" class="btn bg-gray-100 hover:bg-gray-200">مسح الملفات المؤقتة</button>
      ...
      <input type="date"   name="duration_from"    class="hidden">
    </form>

    {{-- Real PDF preview --}}
    <div class="bg-white p-4 rounded shadow space-y-4">
      <div class="text-sm font-semibold">المعاينة الحقيقية (PDF):</div>
...
(function(){
  ...
  // Convert mm-rect into % with safe fallbacks/clamping
  /**
   * Convert template rect in millimeters -> percentages of page size.
   * Handles both LTR (left/width) and RTL (right/width) definitions,
   * and also bottom/height for vertical positioning.
   */
  function mmRectToPercent(r, page){
    const W = page.w, H = page.h;

    // ---- width / height (resolve first so we can derive left/top from right/bottom) ----
    let widthMM;
    if (r.width !== undefined) {
      ...
      widthMM = W * 0.30; // sensible default
    }
    ...
    if (r.height !== undefined) {
      ...
      heightMM = H * 0.05; // sensible default
    }
    if (!isFinite(heightMM) || heightMM <= 0) heightMM = H * 0.05;

    // ---- left / top (support RTL via right; support bottom) ----
    let leftMM;
    if (r.left !== undefined) {
      leftMM = +r.left;
    } else if (r.right !== undefined) {
      leftMM = W - (+r.right) - widthMM;   // <-- RTL support
    } else {
      leftMM = 0;
    }
    ...
    if (r.top !== undefined) {
      ...
      topMM = H - (+r.bottom) - heightMM;  // <-- bottom support
    } else {
      topMM = 0;
    }
    ...
    // ---- convert to percentages & final fit ----
    let lp = c01(leftMM / W);
    let tp = c01(topMM  / H);
  ...
  /**
   * Convert percentages back to millimeters for database storage
   * This reverses the mmRectToPercent conversion
   */
  function percentToMm(pct, page){
    ...
    // Try to determine if we should use 'right' positioning (for RTL fields)
    // Check original template to see if this field uses 'right' instead of 'left'
    const t = currentTpl();
    ...
    // Use right or left based on original template
    if (usesRight) {
    ...
    // Preserve font size if it exists
  ...
  function localKey(){ return 'stud-pos:'+selTrack.value+':'+selGender.value; }
  ...
  function rebuildFromTemplate(forceTemplate=false){
    ...
    // Inspector view of the "true" defaults coming from the template
    inspectorDump(t.positions || {}, page);

    // Load from local cache unless forced / ignored
    ...
    // Build from template defaults (mm -> %)
    removeBoxes();
    ...
    savePositions(true); // don't overwrite cache unless user moves something
  }

  // Build once background is sized
  bgImg.addEventListener('load', ()=>{
    rebuildFromTemplate(true); // ensure we start from template defaults on first load
  ...
  function savePositions(silent=false){
    ...
    // Save percent format to hidden input (for zoom functionality)
  ...
  /**
   * Convert current positions from percent to mm format before form submission
   */
  function convertPositionsToMm(){
    const t = currentTpl();
    const page = (t && t.page_mm) || {w:297, h:210};

    // Get current percent positions
    const pctPositions = JSON.parse(byId('custom_positions').value || '{}');
    const mmPositions = {};

    for (const [key, pct] of Object.entries(pctPositions)) {
      // Add key for template lookup
      pct.key = key;

      // Get font size from advanced settings if available
    ...
    // Update hidden input with mm format for database
  ...
  byId('btn-defaults').addEventListener('click', ()=>{
    // Force reload from template defaults (ignore any saved positions)
    rebuildFromTemplate(true);
  });

  byId('btn-clear-server').addEventListener('click', ()=>{
    const f = document.createElement('form');
    f.method='POST'; f.action='{{ route('students.admin.clear') }}';
  ...
  // Remove ZIP File
  byId('btn-remove-zip').addEventListener('click', ()=>{
    const zipInput = byId('images_zip');
    if (zipInput) {
      zipInput.value = ''; // Clear the file input
      showFlash('تم إزالة ملف الصور المحدد.', 'success');
    }
  });
  ...
  byId('btn-preview').addEventListener('click', async ()=>{
    savePositions();
    const pctBackup = byId('custom_positions').value; // Backup percent format
    convertPositionsToMm(); // Convert to mm format for preview
    flash.classList.add('hidden');
    const fd = new FormData(form);
    byId('custom_positions').value = pctBackup; // Restore percent format for UI
    try{
      const res = await fetch('{{ route('students.admin.preview') }}', {
        method:'POST',
        body:fd,
        headers:{
      ...
      if (!res.ok){
        ...
        // Clone response so we can try both json and text
  ...
  byId('btn-preview-image').addEventListener('click', async ()=>{
    ...
    byId('custom_positions').value = pctBackup; // Restore percent format for UI
    try{
      const res = await fetch('{{ route('students.admin.preview.image') }}', {
        method:'POST',
        body:fd,
        headers:{
          'X-Requested-With':'fetch',
  ...
  // Generate Images (ZIP)
  byId('btn-generate-images').addEventListener('click', async ()=>{
    if (!confirm('هل تريد إنشاء شهادات صور لجميع الطلاب؟')) return;
    savePositions();
    convertPositionsToMm(); // Convert to mm for database
    flash.classList.add('hidden');
    const fd = new FormData(form);
    try{
      const res = await fetch('{{ route('students.admin.store.images') }}', {
        method:'POST',
        body:fd,
        headers:{
          'X-Requested-With':'fetch',
      ...
      if (!res.ok){
        let msg='حدث خطأ أثناء إنشاء الصور.';
        try {
          const j = await res.json();
          if (j?.message) msg=j.message;
        } catch {}
      ...
      if (result.success && result.download_url) {
        ...
        showFlash(result.message || 'تم إنشاء الصور بنجاح!');
        // Start download after a brief delay to show the message
        setTimeout(() => {
    ...
    } catch(e){ console.error(e); showFlash('حدث خطأ غير متوقع أثناء إنشاء الصور.'); }
  });

  // Include positions when generating (convert to mm format)
  ...
  let savedSettings = null; // Store the original DB settings for reset
  ...
  async function loadSavedSettings() {
    ...
    try {
      const url = '{{ route('template.info') }}?role=student&track_key=' + encodeURIComponent(trackKey) + '&gender=' + encodeURIComponent(gender);
      const res = await fetch(url, { headers: { 'Accept': 'application/json' } });
      if (!res.ok) {
        console.warn('Failed to load saved settings:', res.statusText);
        return;
      }
      const data = await res.json();
      if (!data.success) return;

      savedSettings = data; // Store for reset functionality

      // Apply saved settings to advanced options
      ...
      document.querySelectorAll('tr[data-adv-row]').forEach(row => {
        ...
        // Set size from positions (font property)
  ...
  // Load settings when track or gender changes
  ...
  byId('btn-save-defaults').addEventListener('click', async ()=>{
    savePositions();
    const pctBackup = byId('custom_positions').value; // Backup percent format
    convertPositionsToMm(); // Convert to mm format for database
    const fd = new FormData(form);
    byId('custom_positions').value = pctBackup; // Restore percent format for UI
    // Remove files for save operation
    fd.delete('students_file');
    fd.delete('images_zip');

    try {
      const res = await fetch('{{ route('students.admin.save') }}', {
        method: 'POST',
        body: fd,
  ...
  <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
    <div class="p-6">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-xl font-bold">إضافة مسار جديد للطلاب</h2>
        <button type="button" onclick="closeAddTrackModal()" class="text-gray-500 hover:text-gray-700 text-2xl">&times;</button>
      </div>

      <form action="{{ route('students.admin.tracks.add') }}" method="POST" enctype="multipart/form-data">
        @csrf
...
<script>
  ...
  function openEditTrackModal() {
    ...
    form.action = `/students/admin/tracks/${trackId}`;

    // Fetch track data from server
    fetch(`/students/admin/tracks/${trackId}`, {
      headers: {
        'Accept': 'application/json',

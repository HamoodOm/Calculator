                    ...
                    @foreach(\App\Models\ApiClient::getAvailableCertificateTypes() as $type => $label)
                        <label class="flex items-center">
                            <input type="checkbox" name="allowed_certificate_types[]" value="{{ $type }}"
                                {{ in_array($type, old('allowed_certificate_types', $apiClient->allowed_certificate_types ?? [])) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <input type="number" name="max_per_request" id="max_per_request"
                        value="{{ old('max_per_request', $apiClient->max_per_request ?? 1) }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        min="1" max="500">
                ...
                <div>
                    ...
                    <input type="email" name="contact_email" id="contact_email"
                        value="{{ old('contact_email', $apiClient->contact_email) }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            ...
            <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                <label class="flex items-start">
                    <input type="checkbox" name="expose_download_url" value="1"
                        {{ old('expose_download_url', $apiClient->expose_download_url ?? true) ? 'checked' : '' }}
                ...
                <label class="flex items-start">
                    <input type="checkbox" name="require_webhook_success" value="1"
                        {{ old('require_webhook_success', $apiClient->require_webhook_success ?? false) ? 'checked' : '' }}
                        class="mt-0.5 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <div class="mr-3">
